...
const layerLabel = (key: string, label: string, isV2: boolean) => !isV2 ? label : key === 'almRaw' ? '未条件化 x̂₀（纯轨迹主干）' : key === 'almRegions' ? '验证过的凸区域' : label;
...
export default function Home() {
  ...
  return <main className="min-h-screen bg-background text-foreground">
    <header className="topbar"><div className="brand-mark"><Sparkles size={18}/></div><div><h1>Diffusion Lens</h1><p>{isV2?'Skeleton-Topology-Grounded Trajectory Diffusion':'Joint P / E reverse process viewer'}</p></div><div className={`status-pill ${backendReady?'online':'offline'}`}><span/> {backendReady?'GPU SERVICE READY':'SERVICE OFFLINE'}</div></header>

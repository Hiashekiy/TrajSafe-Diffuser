...
export default function Home() {
  ...
  const generate = async () => {
    ...
    try {
      const response = await fetch(`${API}/generate`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sample_key: sample.key, split, dataset: datasetId, model_id: modelId, seed, condition, obstacles: obstacles.map((item)=>[item.x,item.y,item.r]), alm_enabled: almEnabled, steps: denoiseSteps }) });

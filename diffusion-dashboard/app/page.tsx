...
type V2Frame = { t: number; committed: boolean; selectedIdx: number; pi: number[]; regions: AlmRegion[]; progress: number[] | null };
type V2Info = { engine: string; epoch: number | null; commitT: number; selection: string; numCandidates: number; numValid: number; rawK: number; selectedIdx: number; pi: number[]; topologyPath: Vec[]; candidateLengths: number[]; nodePath: number[]; frames: (V2Frame | null)[]; metrics: { trajCollision: boolean; ellipseCount: number; ellipseCollisionRate: number | null; centerFreeRate: number | null; minCenterClearanceCells: number | null; regionCount: number; progressMonotonic: boolean; stepJitter?: number; selectionChanges?: number }; skeleton: { nodes: number; branches: number }; candidatePaths?: Vec[][]; candidateMask?: boolean[]; geometryPoints?: number; stepJitter?: number; selectionChanges?: number };
type Generation = { sampleKey: string; modelId: string; seed: number; cacheHit: boolean; elapsedMs: number; stateLabels: string[]; schedule: { sqrtAlphaBar: number[]; sqrtOneMinusAlphaBar: number[] }; PHistory: Vec[][]; E6History: number[][][]; X0PHistory: Vec[][]; X0E6History: number[][][]; alm: { enabled: boolean; startT: number; frames: (AlmFrame | null)[] } | null; v2?: V2Info | null; error?: string };
...
const layerConfig = [
  ...
  { key: 'almRaw', label: 'ALM 修正前 x̂₀', color: '#ff5ebf' }, { key: 'almRegions', label: 'ALM 凸区域', color: '#2fb9ff' },
...
export default function Home() {
  ...
  const [almEnabled, setAlmEnabled] = useState(true);
  ...
  const [layers, setLayers] = useState<Record<string, boolean>>({ state: true, ellipses: true, waypoints: true, result: false, groundTruth: false, map: true, almRaw: true, almRegions: true, topology: true });
  ...
  const v2Info = history?.v2 ?? null;
  const v2Frame = v2Info?.frames?.[safeStep] ?? null;
  const isV2 = v2Info !== null;
  const isV3 = !!v2Info?.engine?.startsWith('v3');

  useEffect(() => { fetch(`${API}/health`).then((response) => { if (response.ok) setBackendReady(true); }).catch(() => setBackendReady(false)); }, []);
  useEffect(() => { if(history && (!history.X0PHistory?.length || history.alm === undefined)){ setHistory(null); setStatus('idle'); setStatusText('缓存格式已升级，请重新生成'); setReverseStep(0); } }, [history]);
  ...
  const generate = async () => {
    ...
    try {
      const response = await fetch(`${API}/generate`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sampleKey, modelId, seed, condition, obstacles: obstacles.map((item)=>[item.x,item.y,item.r]), almEnabled }) });
  ...
  return <main className="min-h-screen bg-background text-foreground">
    <header className="topbar"><div className="brand-mark"><Sparkles size={18}/></div><div><h1>Diffusion Lens</h1><p>{isV3?'V3 TrajSafe-Diffuser（每步重选骨架）':isV2?'Skeleton-Topology-Grounded Trajectory Diffusion':'Joint P / E reverse process viewer'}</p></div><div className={`status-pill ${backendReady?'online':'offline'}`}><span/> {backendReady?'GPU SERVICE READY':'SERVICE OFFLINE'}</div></header>
    <div className="workspace">
      ...
      <section className="stage-column">
        ...
        <div className={`canvas-shell ${editMode!=='none'?'editing':''}`}><svg viewBox="0 0 256 256" role="img" aria-label="真实占据地图上的联合轨迹与椭圆扩散状态" onClick={handleCanvasClick}><rect width="256" height="256" fill="#d8dde1"/>{layers.map&&<g>{map.wallRuns.map(([x,y,w],i)=><rect key={i} x={x} y={y} width={w} height="1.15" fill="#10161b" opacity=".96"/>)}</g>}<g>{obstacles.map((item,i)=>{const q=xy([item.x,item.y]);return <circle key={i} cx={q[0]} cy={q[1]} r={item.r*128} fill="#10161b" stroke="#ff6b72" strokeWidth=".8"/>;})}</g>{layers.groundTruth&&<polyline points={points(sample.groundTruth.P)} fill="none" stroke="#ffcf5a" strokeWidth="1.15" strokeDasharray="3 2" opacity=".9"/>}{layers.result&&finalState&&<polyline points={points(finalState.P)} fill="none" stroke="#c7ff4a" strokeWidth="1.25" opacity=".9"/>}{displayMode==='compare'&&layers.state&&noisyState&&<polyline points={points(noisyState.P)} fill="none" stroke="#50e3ff" strokeWidth=".8" strokeLinejoin="round" opacity=".48"/>}{isV2&&v2Info&&layers.topology&&<g>{v2Info.candidatePaths?.map((path, i)=><polyline key={`cand-${i}`} points={points(path)} fill="none" stroke="#4aa3ff" strokeWidth=".6" strokeDasharray="1.5 2.2" strokeLinejoin="round" opacity={i===v2Info.selectedIdx?0.0:(v2Info.candidateMask?.[i]===false?0.12:0.3)}/>)}<polyline points={points(v2Info.topologyPath)} fill="none" stroke="#4aa3ff" strokeWidth="1.15" strokeDasharray="2.5 2" strokeLinejoin="round" opacity=".95"/></g>}{almFrame&&displayMode!=='state'&&layers.almRegions&&<g>{almFrame.regions.map((r)=>{const q=r.polygon.map(xy);return <polygon key={r.i} points={q.map((p)=>p.join(',')).join(' ')} fill="#25c8ff" fillOpacity=".08" stroke="#2fb9ff" strokeWidth=".55" strokeOpacity=".9"/>;})}</g>}{almFrame&&displayMode!=='state'&&layers.almRaw&&<polyline points={points(almFrame.rawP)} fill="none" stroke="#ff5ebf" strokeWidth="1.05" strokeDasharray="4 2.4" strokeLinejoin="round" opacity=".9"/>}{layers.ellipses&&<g>{ellipseItems.filter((e)=>e.idx%8===0).map((e)=>{const c=ellipseColor(e.idx,current?.P.length ?? 128);return <ellipse key={e.idx} cx={e.center[0]} cy={e.center[1]} rx={e.a} ry={e.b} transform={`rotate(${e.angle} ${e.center[0]} ${e.center[1]})`} fill={c.fill} fillOpacity=".09" stroke={c.stroke} strokeWidth=".75" opacity=".92"/>;})}</g>}{layers.state&&current&&<polyline points={points(current.P)} fill="none" stroke={displayMode==='state'?'#50e3ff':'#c7ff4a'} strokeWidth={displayMode==='compare'?'1.6':'1.35'} strokeLinejoin="round" opacity=".92"/>}{layers.waypoints&&current&&<g>{current.P.map((p,i)=>{const q=xy(p);return <circle key={i} cx={q[0]} cy={q[1]} r="1.05" fill={displayMode==='state'?'#eaf7fa':'#edffc7'} opacity=".82"/>;})}</g>}<g><circle cx={xy(condition[0])[0]} cy={xy(condition[0])[1]} r="3.4" fill="#55f17a" stroke="#071009" strokeWidth="1"/><path d={`M${xy(condition[1])[0]-4},${xy(condition[1])[1]}h8M${xy(condition[1])[0]},${xy(condition[1])[1]-4}v8`} stroke="#ff5d67" strokeWidth="2"/></g></svg>{!history&&editMode==='none'&&<div className="generation-empty"><Sparkles/><strong>等待生成</strong><span>可先点击编辑起终点与障碍<br/>再运行一次模型</span><Button disabled={!backendReady} onClick={generate}>生成扩散序列</Button></div>}<div className="canvas-badge">{editMode!=='none'?`编辑：${editMode==='start'?'起点':editMode==='goal'?'终点':editMode==='obstacle'?'添加障碍':'擦除障碍'}`:history?(stateLabel==='x0'?'final x₀':displayMode==='state'?<>x<sub>{t}</sub> state</>:<>x̂<sub>0</sub><sup>({t})</sup></>):'no sequence'}</div><div className="axis-label x">scene x ∈ [-1, 1]</div><div className="axis-label y">scene y ∈ [-1, 1]</div></div>
        ...
        <p className="method-note">“x₀ prediction”展示每个 timestep 模型刚输出的 P̂₀⁽ᵗ⁾ / Ê₀⁽ᵗ⁾；“noisy state”展示送入模型的 Pₜ / Eₜ。V3 每步重选骨架候选：浅蓝虚线是当前 occupancy/起终点下的搜索路径，深蓝实线是选中的拓扑。V3 开启凸区域修正时，t≤start_t 的帧会叠加预测椭圆生成的 verified convex region（青色）和修正前 x̂₀（粉色虚线）；关闭时粉色虚线为主干 coarse x̂₀。椭圆颜色由浅到深表示它们沿轨迹的顺序（起点→终点）。</p>
      </section>

      <aside className="right-rail"><div className="metric-title"><Activity size={15}/> 当前扩散状态</div><div className="hero-metric"><span>{displayMode==='state'?'noisy state':displayMode==='prediction'?'x₀ prediction':'prediction + state'}</span><strong>{stateLabel}<small>{history&&stateLabel==='x0'?' · final':history?' / 15':''}</small></strong><div className="meter"><i style={{width:`${history?safeStep/maxStep*100:0}%`}}/></div></div><div className="metric-grid"><div><span>√ᾱₜ</span><strong>{history?(stateLabel==='x0'?1:history.schedule.sqrtAlphaBar[t]).toFixed(4):'—'}</strong></div><div><span>√(1−ᾱₜ)</span><strong>{history?(stateLabel==='x0'?0:history.schedule.sqrtOneMinusAlphaBar[t]).toFixed(4):'—'}</strong></div><div><span>{displayMode==='state'?'Pₜ → P₀ RMSE':'P̂₀⁽ᵗ⁾ → P₀ RMSE'}</span><strong>{rmse===null?'—':rmse.toFixed(3)}</strong></div><div><span>碰撞 / 越界点</span><strong>{collisionCount===null?'—':`${collisionCount} / 128`}</strong></div><div><span>当前路径长度</span><strong>{current?pathLength(current.P).toFixed(2):'—'}</strong></div><div><span>缓存状态</span><strong>{history?(history.cacheHit?'HIT':'SAVED'):'—'}</strong></div></div><div className="step-log"><div className="metric-title"><Gauge size={15}/> {v2Frame?(isV3?'V3 安全诊断':'V2 安全诊断'):almFrame?'ALM 修正诊断':'任务状态'}</div>{v2Info&&v2Frame&&<div className="metric-grid"><div><span>{isV3?'每步重选 m':'commit t / 选中 m'}</span><strong>{isV3?`每步 / ${v2Frame.selectedIdx}`:`${v2Info.commitT} / ${v2Frame.selectedIdx}`}</strong></div><div><span>π(m)</span><strong>{v2Frame.pi[v2Frame.selectedIdx]?.toFixed(3) ?? '—'}</strong></div><div><span>候选数 valid / total</span><strong>{v2Info.numValid} / {v2Info.numCandidates}</strong></div><div><span>中心安全率 CenterFree</span><strong>{v2Info.metrics.centerFreeRate===null?'—':(v2Info.metrics.centerFreeRate*100).toFixed(2)+'%'}</strong></div><div><span>中心最小余量</span><strong>{v2Info.metrics.minCenterClearanceCells===null?'—':v2Info.metrics.minCenterClearanceCells.toFixed(2)+' 格'}</strong></div><div><span>轨迹碰撞</span><strong>{v2Info.metrics.trajCollision?'有':'无'}</strong></div><div><span>椭圆点碰撞率</span><strong>{v2Info.metrics.ellipseCollisionRate===null?'—':(v2Info.metrics.ellipseCollisionRate*100).toFixed(2)+'%'}</strong></div><div><span>本帧验证凸区域</span><strong>{v2Frame.regions?.length ?? 0}</strong></div><div><span>进度单调</span><strong>{v2Info.metrics.progressMonotonic?'是':'否'}</strong></div><div><span>骨架 nodes / branches</span><strong>{v2Info.skeleton.nodes} / {v2Info.skeleton.branches}</strong></div></div>}{almFrame&&almFrame.stats.violation_before!==undefined&&<div className="metric-grid"><div><span>违约率 before → after</span><strong>{almFrame.stats.violation_before.toFixed(4)} → {almFrame.stats.violation_after.toFixed(4)}</strong></div><div><span>修正量 mean / max</span><strong>{almFrame.stats.mean_correction.toFixed(4)} / {almFrame.stats.max_correction.toFixed(4)}</strong></div><div><span>激活约束率</span><strong>{(almFrame.stats.raw_max_positive_rate*100).toFixed(1)}%</strong></div><div><span>λ mean / max</span><strong>{almFrame.stats.lambda_mean.toFixed(4)} / {almFrame.stats.lambda_max.toFixed(4)}</strong></div><div><span>平滑度 before → after</span><strong>{almFrame.stats.smoothness_before.toFixed(3)} → {almFrame.stats.smoothness_after.toFixed(3)}</strong></div><div><span>引导段（碰撞）</span><strong>{almFrame.enforced.length} / {almFrame.rawP.length-1}</strong></div></div>}<div className="phase"><i/>{statusText}</div><p>{history?(displayMode==='state'?'当前显示 sampler 输入的 noisy state。':almFrame?'该帧处于 ALM 引导范围：亮青为修正后 x̂₀（实际进入下一步 DDIM），洋红虚线为修正前，青色多边形为修正所用的凸区域。':'当前显示模型在该 timestep 直接预测的 clean x₀，可用于判断轨迹何时成型。'):'模型尚未运行；地图与 Ground Truth 可先用于确认样本。'}</p></div><div className="provenance"><Box size={14}/><div><strong>推理来源</strong><span>{data.provenance.dataset}<br/>{data.provenance.checkpoints[modelId]}</span></div></div></aside>
    </div>
  </main>;
}

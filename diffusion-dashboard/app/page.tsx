...
type Sample = { key: string; maze: string; datasetId: number; condition: Vec[]; groundTruth: { P: Vec[] } };
...
type AlmFrame = { t: number; raw_p: Vec[]; enforced: number[]; regions: AlmRegion[]; stats: Record<string, number> };
...
const models = [
  { id: 'best', name: 'TrajSafe · Best (ep 19)', source: 'ckpt / best.pt' },
  { id: 'latest', name: 'TrajSafe · Latest (ep 100)', source: 'ckpt / latest.pt' },
];
...
const layerConfig = [
  ...
  { key: 'rawX0', label: '修正前 x̂₀ / 轨迹主干', color: '#ff5ebf' }, { key: 'regions', label: '验证凸区域（ALM 修正）', color: '#2fb9ff' },
...
export default function Home() {
  ...
  const [layers, setLayers] = useState<Record<string, boolean>>({ state: true, ellipses: true, waypoints: true, result: false, groundTruth: false, map: true, rawX0: true, regions: true, topology: true });
  ...
  useEffect(() => { if(history && (!history.x0_history?.length || history.alm === undefined)){ setHistory(null); setStatus('idle'); setStatusText('缓存格式已升级，请重新生成'); setReverseStep(0); } }, [history]);
  ...
  const generate = async () => {
    ...
    try {
      ...
      setHistory(result); setReverseStep(0); setStatus('ready'); setStatusText(result.cache_hit ? `缓存命中 · ${result.elapsed_ms} ms` : `GPU 生成并已缓存 · ${result.elapsed_ms} ms`);
  ...
  return <main className="min-h-screen bg-background text-foreground">
    <header className="topbar"><div className="brand-mark"><Sparkles size={18}/></div><div><h1>Diffusion Lens</h1><p>{'TrajSafe-Diffuser · 骨架引导的安全扩散规划（每步重选骨架）'}</p></div><div className={`status-pill ${backendReady?'online':'offline'}`}><span/> {backendReady?'GPU SERVICE READY':'SERVICE OFFLINE'}</div></header>
    <div className="workspace">
      <aside className="left-rail">
        <section><div className="section-label"><Database size={14}/> 数据源</div><div className="source-card"><strong>scenes</strong><span>test · 256×256 · H=128</span></div></section>

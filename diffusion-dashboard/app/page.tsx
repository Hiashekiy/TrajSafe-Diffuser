...
// One processed cache the panel can display.  160k8p (k=8, ~+10 m channels) is
// the campaign cache; 160k4p (k=4, ~+5 m) is the stricter TEST-ONLY map, so
// `splits` is what actually exists on disk and the split selector follows it.
type DatasetEntry = { id: string; label: string; note?: string; root: string; splits: string[]; counts?: Record<string, number> };
...
type AlmInfo = { final_project?: boolean; enabled: boolean; mode: string; start_t: number; warmup_reverse_steps: number; activation_step: number; frozen_topology_idx: number; status: string; rho: number; constraint_tol: number; max_curve_step_scene: number; frames: (AlmFrame | null)[] };
...
type FinalProjection = { status: string | null; applied: boolean; feasible: boolean; violation_before: number | null; violation_after: number | null; correction_m: number | null };
type Generation = { sample_key: string; model_id: string; seed: number; cache_hit: boolean; elapsed_ms: number; steps?: number; times?: number[]; state_labels: string[]; schedule: { sqrt_alpha_bar: number[]; sqrt_one_minus_alpha_bar: number[] }; state_history: Vec[][]; x0_history: Vec[][]; control_history?: ControlFrame[]; ellipse_history: EllipseFrame[]; alm: AlmInfo | null; corridor?: Corridor | null; activation?: ActivationInfo | null; pack_summary?: PackSummary | null; final_validation?: FinalValidation | null; final_projection?: FinalProjection | null; pre_projection?: Vec[] | null; final_curve?: Vec[] | null; x0_raw_history?: Vec[][]; topology?: TopologyInfo | null; error?: string };
...
// Model ids MUST match ``CHECKPOINTS`` in engine_carla.py ("<run>:<kind>").
// A/B2 were trained WITH the historical safety feedback, B1 without (control
// arm), REF is the old 160k8 (k=8) baseline kept for continuity.  All of them
// are displayed on the 160k8p cache (DASH_CONFIG).
const models = [
  { id: 'A_oneshot:best_task', name: 'A · 一步到位 feedback · best_task(ep50)', source: 'campaign_a_oneshot · 从零直接开 feedback（58 ep）' },
  { id: 'A_oneshot:latest', name: 'A · 一步到位 feedback · latest(ep57)', source: 'campaign_a_oneshot · 墙钟截断时的最后状态' },
  { id: 'B2_feedback:best_task', name: 'B2 · 基座+feedback 微调 · best_task(ep36)', source: 'campaign_b2_feedback · 100 ep 基座后再微调 37 ep' },
  { id: 'B2_feedback:latest', name: 'B2 · 基座+feedback 微调 · latest(ep62)', source: 'campaign_b2_feedback · 最后状态' },
  { id: 'B1_base:best_task', name: 'B1 · 基座（无 feedback） · best_task(ep24)', source: 'campaign_b1_base · 对照臂，feedback 关闭' },
  { id: 'B1_base:latest', name: 'B1 · 基座（无 feedback） · latest(ep99)', source: 'campaign_b1_base · 最后状态' },
  { id: 'REF_160k8:best_task', name: 'REF · 旧 160k8 best_task(ep175)', source: 'bspline_carla_160k8 · 跨缓存参考' },
  ...
  // C=32 -> 48 controls, same k=4 cache / same recipe.  请配 160k4p_c48 缓存
  // （只有那个缓存的 GT 控制点是 48 点）。test 9/420（ALM 开）不如 K4P 的 5/420，
  // 但 7 个碰撞全在 Town10HD 同一场景；Town01 上反而更好，详见 §12。
  { id: 'K4P_c48_oneshot:best_task', name: 'K4P_c48 · 48 控制点重训 · best_task(ep55)', source: 'oneshot_k4p_c48 · 200 ep / 7.4 h，需配 160k4p_c48 缓存（test 9/420，集中在 Town10HD）' },
  { id: 'K4P_c48_oneshot:latest', name: 'K4P_c48 · 48 控制点重训 · latest(ep200)', source: 'oneshot_k4p_c48 · 最后状态（不建议用，拓扑项已过拟合）' },
];
...
const layerConfig = [
  ...
  { key: 'waypoints', label: '128 个 Waypoint', color: '#eef2f5' }, { key: 'controls', label: '预测的控制点', color: '#ff9f1c' }, { key: 'gtControls', label: 'GT 的控制点（固定）', color: '#c08a2e' }, { key: 'result', label: '模型最终输出 P₀', color: '#c7ff4a' },
  { key: 'groundTruth', label: '数据集 Ground Truth', color: '#ffcf5a' }, { key: 'map', label: 'Occupancy Map', color: '#65727e' },
  { key: 'rawX0', label: 'raw x̂₀（修正前，洋红）/ ALM safe x̂₀（青色）', color: '#ff5ebf' }, { key: 'preProjection', label: '硬投射前（引导阶段输出，橙色虚线）', color: '#ff9f1c' }, { key: 'regions', label: '冻结安全走廊 · 128 基础区域（cyan）/ bridge（橙色）', color: '#2fb9ff' },
...
export default function Home() {
  ...
  const [datasetCatalog, setDatasetCatalog] = useState<DatasetEntry[]>([]);
  ...
  // Reverse forwards actually executed: 4 is the campaign default (measured on
  // the full test split: 4 and 16 steps give the SAME collision rate, see
  // docs/CAMPAIGN_160K8P_RESULTS.md), 16 = the full training schedule.
  const [denoiseSteps, setDenoiseSteps] = useState(4);
  const [almEnabled, setAlmEnabled] = useState(false);
  // 末端硬投射开关：false 时返回引导阶段的原始输出（不做 LP 投影）
  ...
  const datasetEntry = datasetCatalog.find((item) => item.id === datasetId) ?? null;
  // 160k4p ships test only: the split selector must only offer what exists.
  const availableSplits: string[] = datasetEntry?.splits?.length ? datasetEntry.splits : [...SPLITS];
  const datasetLabel = datasetEntry?.label ?? datasetId;
  ...
  // the authoritative curve: the payload's post-hard-projection output;
  // state_history stops at the last reverse step, i.e. BEFORE the projection
  const finalState = history ? { P: (history.final_curve ?? history.state_history.at(-1)!) } : null;
  ...
  const finalProjection = history?.final_projection ?? null;
  ...
  useEffect(() => { fetch(`${API}/health`).then((r) => r.ok ? r.json() : Promise.reject(new Error(String(r.status)))).then((raw: unknown) => { const info = (raw ?? {}) as { engine?: string }; setBackendReady(true); setBackendEngine(String(info.engine ?? 'legacy')); }).catch(() => { setBackendReady(false); setBackendEngine(''); }); }, []);
  const engineMismatch = backendReady && !backendEngine.startsWith('carla-controlspace');
  ...
  const loadSample = async (nextSplit: string, index: number, nextDataset = datasetId) => {
    ...
    try {
      const response = await fetch(`${API}/sample?dataset=${encodeURIComponent(nextDataset)}&split=${encodeURIComponent(nextSplit)}&index=${Math.max(0, Math.trunc(index))}`);
      const payload = (await response.json()) as Sample & { error?: string };
  ...
  // Switching the cache swaps the MAP under the same sample ids, so the loaded
  // sample and every generated frame are invalid: reload index 0 of the first
  // split that cache actually has (and drop the stale sequence).
  const changeDataset = (next: string) => {
    const entry = datasetCatalog.find((item) => item.id === next);
    const nextSplit = entry?.splits?.length ? (entry.splits.includes(split) ? split : entry.splits[0]) : split;
    setDatasetId(next); setSplit(nextSplit);
    if (entry?.counts) setSplitCounts(entry.counts);
    if (entry?.root) setDatasetRoot(entry.root);
    invalidate();
    void loadSample(nextSplit, 0, next);
  };
  ...
  useEffect(() => {
    let alive = true;
    fetch(`${API}/splits`).then((r) => r.ok ? r.json() : Promise.reject(new Error(String(r.status)))).then((raw: unknown) => { const info = (raw ?? {}) as { dataset?: string; datasets?: DatasetEntry[]; splits?: SplitEntry[]; provenance?: { dataset?: string } }; if (!alive) return; const entries = (info.datasets ?? []) as DatasetEntry[]; setDatasetCatalog(entries); const active = entries.find((item) => item.id === (info?.dataset ?? DEFAULT_DATASET)) ?? entries[0]; const counts: Record<string, number> = active?.counts ?? (() => { const fallback: Record<string, number> = {}; for (const entry of (info?.splits ?? []) as SplitEntry[]) fallback[String(entry.name)] = Number(entry.count) || 0; return fallback; })(); setSplitCounts(counts); const ds = String(active?.root ?? info?.provenance?.dataset ?? ''); if (ds) setDatasetRoot(ds); }).catch(() => {});
  ...
  const generate = async () => {
    ...
    try {
      const response = await fetch(`${API}/generate`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ sample_key: sample.key, split, dataset: datasetId, model_id: modelId, seed, condition, obstacles: obstacles.map((item)=>[item.x,item.y,item.r]), alm_enabled: almEnabled, final_project: finalProject, steps: denoiseSteps }) });

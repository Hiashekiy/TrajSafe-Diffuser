...
- 通过“ALM 修正”标签回放最终 `x₀` 的椭圆中心递推、凸区域生成和轨迹 ALM inner steps。
- 使用虚线原始椭圆与实线修正椭圆对照，并通过左右方向键或空格控制回放。
...
先在“扩散过程”标签生成序列，再切换到“ALM 修正”并点击“生成 ALM 修正序列”。回放分为两段：

1. 依次处理 128 个椭圆中心；洋红虚线是原始椭圆，亮青实线是递推投影后的椭圆，深蓝色块和粗青边框是当前凸区域。
2. 中心递推结束后，依次显示轨迹进入 ALM 前的状态和每个 inner iteration 的修正结果；洋红虚线是修正前轨迹，亮青实线是当前轨迹。

可使用播放按钮、时间轴和速度按钮，也可按 `←`、`→` 单步切换，按空格播放或暂停。
